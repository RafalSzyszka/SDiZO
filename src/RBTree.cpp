...
//-------------------------------------------
void RBTree::DFSRelease(RBTNode * p)
{
  if(p != &S)
  {
...
RBTree* RBTree::initFromFile(string filename) {
    ...
    if(file.is_open()) {
        while (getline(file, line)) {
            int value;
            stringstream iss(line);   //converting string to int
            iss >> value;
            rbtree->insertRBT(value);
        }
        return rbtree;
    } else {
        cout << "Blad podczas otwierania pliku, struktura pozostaje pusta." << endl;
        return new RBTree();
    }
}
